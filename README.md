...
The above commands write the output to standard out.

```bash
$ python3 -m pel.peltool.peltool -d <directory> -o <output directory>
```

The above command parses every PEL in the directory passed in and writes the
output to `<output directory>/<original filename>.<PEL EID>.json`.  It will
skip files that aren't PELs, though the '-e' option can be used to only look at
